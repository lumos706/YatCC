...
Ast2Asg::operator()(ast::PostfixExpressionContext* ctx)
{
  ...
  for (unsigned i = 1; i < ctx->children.size(); ++i) {
    if (auto bracket = dynamic_cast<antlr4::tree::TerminalNode*>(ctx->children[i])) {
      if (bracket->getSymbol()->getType() == ast::LeftBracket) {
        auto node = make<BinaryExpr>();
        node->op = BinaryExpr::kIndex;
        node->lft = sub;
        node->rht = self(dynamic_cast<ast::ExpressionContext*>(ctx->children[++i]));
        sub = node;
      }
    }
  }
